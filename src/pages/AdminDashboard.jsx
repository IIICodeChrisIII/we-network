  ...
  const fetchDashboardData = async () => {
    ...
    if (profiles) {
      ...
      setStudents(studentsList.map(s => ({
        ...
        lastTouchpoint: s.status === 'intern' ? t('admin.tp_intern') : t('admin.tp_event')
      })));
      setStats({
    ...
    <div className="page-content animate-fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ width: '100%', maxWidth: '1400px' }}>
        <div className="page-header flex-between" style={{ marginBottom: '40px' }}>
          <div>
            <h1 className="page-title" style={{ fontSize: '2.4rem' }}>{t('admin.title')}</h1>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '8px' }}>
              <p className="page-description" style={{ fontSize: '1.1rem', margin: 0 }}>{t('admin.description')}</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'var(--bg-secondary)', padding: '6px 12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                <span style={{ fontSize: '0.9rem', color: 'var(--text-muted)', fontWeight: '500' }}>{t('admin.budget_sim')}</span>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <select className="input-field" value={timeFilter} onChange={e => setTimeFilter(e.target.value)} style={{ padding: '8px 16px', borderRadius: 'var(--border-radius-pill)', backgroundColor: 'var(--bg-secondary)', fontWeight: '600' }}>
              <option value="30d">{t('admin.last_30d')}</option>
              <option value="Q2">{t('admin.q2')}</option>
              <option value="YTD">{t('admin.ytd')}</option>
            </select>
            <button className="btn btn-secondary" style={{ borderRadius: 'var(--border-radius-pill)', padding: '12px 24px', fontSize: '1rem' }}>{t('admin.export_pdf')}</button>
          </div>
        </div>
      ...
      <div className="charts-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px', marginBottom: '48px' }}>
        ...
        <div className="card" style={{ padding: '32px' }}>
          <h3 style={{ marginBottom: '24px', fontSize: '1.4rem' }}>{t('admin.chart_funnel')}</h3>
          <div style={{ height: '400px' }}>
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="card" style={{ marginBottom: '48px' }}>
        <h3 style={{ marginBottom: '24px', fontSize: '1.4rem' }}>{t('admin.pending_docs')}</h3>
        {pendingDocs.length === 0 ? (
          <p className="text-secondary">{t('admin.no_pending_docs')}</p>
        ) : (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: '4px 0 0 0', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                      {doc.type === 'student_verification' ? t('admin.doc_imma') : t('admin.doc_cert')} • {doc.file_name}
                    </p>
                  </div>
                </div>
        ...
        <div className="flex-between" style={{ marginBottom: '24px', gap: '16px', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <select className="input-field" value={filterUni} onChange={e => setFilterUni(e.target.value)} style={{ padding: '8px 12px', width: 'auto' }}>
              <option value="">{t('admin.all_unis')}</option>
            ...
            <div style={{ position: 'relative' }}>
              <input 
                ...
                type="text" 
                list="degrees-list"
                placeholder={t('admin.search_major')}
                value={filterDegree} 
            ...
            <div className="input-with-icon" style={{ width: '240px' }}>
              ...
              <input type="text" className="input-field pl-10" placeholder={t('admin.search_talents')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-color)', textAlign: 'left' }}>
                <th style={{ padding: '12px', color: 'var(--text-secondary)' }}>{t('admin.col_name')}</th>
                <th style={{ padding: '12px', color: 'var(--text-secondary)' }}>{t('admin.col_uni')}</th>
                <th style={{ padding: '12px', color: 'var(--text-secondary)' }}>{t('admin.col_major')}</th>
                <th style={{ padding: '12px', color: 'var(--text-secondary)' }}>{t('admin.col_status')}</th>
                <th style={{ padding: '12px', color: 'var(--text-secondary)' }}>{t('admin.col_engagement')}</th>
                <th style={{ padding: '12px', color: 'var(--text-secondary)' }}>{t('admin.col_touchpoint')}</th>
                <th style={{ padding: '12px', color: 'var(--text-secondary)' }}>{t('admin.col_action')}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={student.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '16px 12px' }}>
                    {student.status === 'intern'
                      ? <span style={{ color: 'var(--accent-green)', fontWeight: '600' }}>{t('admin.status_intern')}</span>
                      : student.status === 'employee' 
                      ? <span style={{ color: 'var(--accent-green)', fontWeight: '600' }}>{t('admin.status_employee')}</span>
                      : <span style={{ background: 'rgba(204,0,0,0.1)', color: 'var(--accent-red)', padding: '4px 8px', borderRadius: '4px', fontSize: '0.85rem' }}>{t('admin.status_pool')}</span>}
                  </td>
                  <td style={{ padding: '16px 12px' }}>
                  ...
                  <td style={{ padding: '16px 12px' }}>
                    <button className="text-accent" onClick={() => setSelectedCrmProfile(student)} style={{ fontSize: '0.9rem', fontWeight: '500', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>{t('admin.crm_profile')}</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {selectedCrmProfile && createPortal(
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="card animate-fade-in" style={{ width: '90%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', position: 'relative', padding: '32px' }}>
            ...
            <h3 style={{ fontSize: '1.1rem', marginBottom: '16px', color: 'var(--text-primary)' }}>{t('admin.career_journey')}</h3>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
                ...
                <div style={{ display: 'flex', gap: '16px' }}>
                  ...
                  <div style={{ paddingBottom: '24px' }}>
                    <h4 style={{ margin: '0 0 4px 0', fontSize: '1rem', color: 'var(--text-primary)' }}>{t('admin.journey_join')}</h4>
                  ...
                  <div style={{ display: 'flex', gap: '16px', animation: 'fadeIn 0.5s ease-out' }}>
                    ...
                    <div style={{ paddingBottom: '24px' }}>
                      <h4 style={{ margin: '0 0 4px 0', fontSize: '1rem', color: 'var(--text-primary)' }}>
                        {selectedCrmProfile.status === 'working_student' ? t('admin.journey_start_working_student') : t('admin.journey_start_intern')}
                      </h4>
                ...
                {selectedCrmProfile.status === 'employee' && (
                  <div style={{ display: 'flex', gap: '16px', animation: 'fadeIn 0.5s ease-out' }}>
                    ...
                    <div>
                      <h4 style={{ margin: '0 0 4px 0', fontSize: '1rem', color: 'var(--text-primary)' }}>{t('admin.journey_employee')}</h4>
                      <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{t('admin.today')}</span>
                    </div>
                  </div>
                )}
